...
#==================lds
def cosvalue(cos_list):
    from torch.nn.functional import cosine_similarity
    x = cos_list[0]
    x_flat = x.view(x.size(0), -1)
    for iterval, y in enumerate(cos_list[1:], start=1):
        y_flat = y.view(x.size(0), -1)
        cos_sim = cosine_similarity(x_flat, y_flat)
        print(f'interval {iterval} cos_sim is: {cos_sim}')
