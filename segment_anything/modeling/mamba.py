...
class PatchMerging2D(nn.Module):   # downsample
...
class VSSMBLOCK(nn.Module):
    def __init__(self, depths=[4],
                 dims=[768], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                 norm_layer=nn.LayerNorm, patch_norm=True,
                 use_checkpoint=False):
        super().__init__()
        self.num_layers = len(depths)  # 4
...
if __name__ == '__main__':
    ...
    x = torch.randn(1, 48, 48, 768).to("cuda:0")
    vsblock = VSSMBLOCK()
    net = vsblock.to("cuda:0")
    print(net(x).shape)
